...
from orientation.classifier import OrientationClassifier
...
def write_scalers(w, x, step, path):
    assert len(x.shape) == 1
    x_dict = dict([(f'{i:02d}', x[i]) for i in range(len(x))])
    w.add_scalars(path, x_dict, global_step=step)
...
class Trainer(object):
    def __init__(self, 
        ...
        # if use uniform distillation score
        if self.opt.usd:
            # initialize classifier
            self.classifier = OrientationClassifier(opt.cls_ckpt, opt.name, self.device)
            self.classifier.init_adaptive_pc(opt.n_tinterval, opt.ema_kiters, opt.ema_previous)
            self.guidance.classifier = self.classifier
        ...
        if not self.opt.test:
            self.time_scheduler = self.get_time_scheduler()

        if self.opt.norm_coef is not None:
            self.opt.norm_coef = torch.tensor(self.opt.norm_coef).to(self.device)
    ...
    def get_time_scheduler(self):
        # truncate noise step 500
        if self.opt.t_schedule == 't5':
            assert self.opt.t5_iters != -1
            sample_intervals = []
            for i in range(self.opt.iters):
                min_step = self.guidance.min_step
                max_step = self.guidance.max_step if (i < self.opt.t5_iters and self.guidance.max_step > 500) else (500 + 1)
                sample_intervals.append([min_step, max_step])
        # back-and-forth expanding and shrinking with lazy activation
        elif 'lazy_bnf' in self.opt.t_schedule:
            num_stages = num_stages = int(self.opt.t_schedule[8:]) if len(self.opt.t_schedule[8:]) > 0 else self.opt.n_tinterval
            interval = self.guidance.num_train_timesteps // num_stages
            if self.global_step != 0:
                self.opt.lazy_activation = self.global_step
                self.guidance.max_step = self.guidance.num_train_timesteps
            assert self.guidance.min_step < interval
            assert self.guidance.max_step > self.guidance.num_train_timesteps - interval
            sample_intervals = []

            if self.opt.lazy_activation is not None and self.opt.lazy_activation > 0:
                active_steps = self.opt.iters - self.opt.lazy_activation
                for _ in range(self.opt.lazy_activation):
                    sample_intervals.append([int((num_stages - 1) * interval), self.guidance.max_step])
            else:
                active_steps = self.opt.iters

            for i in range(num_stages):
                if i == num_stages - 1:
                    min_step = self.guidance.min_step
                else:
                    min_step = int((num_stages - i - 1) * interval)
                max_step = self.guidance.max_step
                for _ in range(active_steps // (2 * num_stages)):
                    sample_intervals.append([min_step, max_step])
            for i in range(num_stages):
                if i == 0:
                    max_step = self.guidance.max_step
                else:
                    max_step = int((num_stages - i) * interval)
                min_step = self.guidance.min_step
                for _ in range(active_steps // (2 * num_stages)):
                    sample_intervals.append([min_step, max_step])
    ...
    def train_step(self, data):
        ...
        text_z = rearrange(text_z, "b c nt nf -> (c b) nt nf")
        ...
        if self.global_step >= len(self.time_scheduler):
            t_schedule = self.time_scheduler[len(self.time_scheduler) - 1]
        else:
            t_schedule = self.time_scheduler[self.global_step]

        if self.opt.lazy_activation is not None and self.global_step < self.opt.lazy_activation:
            step_usd = False
        else:
            step_usd = True

        control_dirs = None
        if self.opt.pose_control:
            control_dirs = data['dir']

        norm_coef = None
        if self.opt.norm_coef is not None:
            if self.global_step < self.opt.norm_iters:
                norm_coef = self.opt.norm_coef
        # encode pred_rgb to latents
        loss, usd_loss, pseudo_loss, latents = self.guidance.train_step(text_z, pred_rgb, self.opt.scale, q_unet, pose, shading = shading, as_latent=as_latent, t_schedule=t_schedule, step_usd=step_usd, dirs=control_dirs, norm_coef=norm_coef)
        ...
        return pred_rgb, pred_depth, loss, usd_loss, pseudo_loss, latents, shading
    ...
    def eval_step_usd(self, preds):
        input_preds = preds.permute(0, 3, 1, 2)
        text_z = self.text_z
        text_z = rearrange(text_z, "b c nt nf -> (c b) nt nf")
        noisy_probs, hat_imgs = self.guidance.eval_step_usd(text_z, input_preds, self.opt.scale)
        return noisy_probs, hat_imgs
    ...
    def train(self, ds, train_loader, valid_loader, max_epochs):
        ...
        if self.use_tensorboardX and self.local_rank == 0:
            ...
            if self.opt.log_particle:
                self.writer_particle = SummaryWriter(os.path.join(self.workspace, "run", self.name, 'log_particle'))
        ...
        for epoch in range(self.epoch + 1, max_epochs + 1):
            ...
            if self.epoch > 1000:
                continue
            if self.opt.usd and self.epoch % self.opt.log_interval == 0:
                for ipc, pca in enumerate(self.classifier.pc_adaptive):
                    write_scalers(self.writer, pca, self.global_step, f"pc_adaptive/pc_interval_{ipc:02d}")
    ...
    def test(self, loader, save_path=None, name=None, write_video=True, idx = 0, shading = None):
        ...
        os.makedirs(save_path, exist_ok=True)
        if not write_video:
            os.makedirs(os.path.join(save_path, 'renders', shading), exist_ok=True)
            if shading == 'albedo':
                os.makedirs(os.path.join(save_path, 'renders', 'depth'), exist_ok=True)
        ...
        with torch.no_grad():
            ...
            for i, data in enumerate(loader):
                ...
                if write_video:
                    ...
                    cv2.imwrite(os.path.join(save_path, 'renders', shading, f'img_{name}_{idx:02d}_{i:06d}_rgb.png'), cv2.cvtColor(pred, cv2.COLOR_RGB2BGR))
                    if shading == "albedo":
                        cv2.imwrite(os.path.join(save_path, 'renders', 'depth', f'img_{name}_{idx:02d}_{i:06d}_depth.png'), pred_depth)
    ...
    def train_one_epoch(self, loader):
        ...
        for data in loader:
            ...
            with torch.cuda.amp.autocast(enabled=self.fp16):
                pred_rgbs, pred_depths, loss, usd_loss, pseudo_loss, latents, shading = self.train_step(data)
            ...
            loss_val = loss.item()
            loss_usd = usd_loss.item()
            loss_pseudo = pseudo_loss.item()

            # write to tensorboard
            if self.use_tensorboardX:
                self.writer.add_scalar('training/vsd_loss', pseudo_loss, global_step=self.global_step)
                if self.opt.usd:
                    self.writer.add_scalar('training/usd_loss', usd_loss, global_step=self.global_step)

            total_loss += loss_val
            total_usd_loss += loss_usd
            ...
            if self.scheduler_update_every_step:
                pbar.set_description(f"loss={loss_val:.4f} ({total_loss/self.local_step:.4f}) usd_loss={loss_usd:.4f} ({total_usd_loss/self.local_step:.4f}) pseudo_loss={loss_pseudo:.4f} ({total_pseudo_loss/self.local_step:.4f}), lr={self.optimizer.param_groups[0]['lr']:.6f}")
            else:
                pbar.set_description(f"loss={loss_val:.4f} ({total_loss/self.local_step:.4f}) usd_loss={loss_usd:.4f} ({total_usd_loss/self.local_step:.4f}) pseudo_loss={loss_pseudo:.4f} ({total_pseudo_loss/self.local_step:.4f})")
            pbar.update(loader.batch_size)
        ...
        average_usd_loss = total_usd_loss / self.local_step
        average_pseudo_loss = total_pseudo_loss / self.local_step

        self.stats["loss"].append(average_loss)
        self.stats["usd_loss"].append(average_usd_loss)
        self.stats["pseudo_loss"].append(average_pseudo_loss)
        ...
        with torch.no_grad():
            ...
            for idx in range(self.opt.val_nz):
                ...
                for data in loader:    
                    ...
                    #     if self.opt.usd and shading == 'albedo':
                    #         noisy_probs, hat_imgs = self.eval_step_usd(preds) # [n_tinterval, n_cluster] [n_tinterval, 3, 512, 512]
                    #         all_noisy_probs.append(noisy_probs)
                    #         all_hat_imgs.append(torch.nn.functional.interpolate(hat_imgs, 64))
            ...
            if self.local_rank == 0:
                ...
                if shading == "albedo":
                    ...
                    # if self.opt.usd:
                    #     all_noisy_probs = torch.stack(all_noisy_probs)
                    #     all_hat_imgs = torch.stack(all_hat_imgs)


                    #     all_hat_imgs = all_hat_imgs.reshape(-1, 3, 64, 64)
                    #     all_hat_imgs = torchvision.utils.make_grid(all_hat_imgs, normalize=True, scale_each=True, nrow=self.classifier.n_tinterval)
                    #     self.writer.add_image('validation/all_nimages', all_hat_imgs, self.global_step)
                    #     # mean for different paticles
                    #     mean_noisy_probs = all_noisy_probs.mean(0)
                    #     # log the mean of particles for each timestep
                    #     for i, mp in enumerate(mean_noisy_probs):
                    #         write_scalers(self.writer, mp, self.global_step, f"validation/interval_{i:02d}")
                    #     # log the mean of particles (global mean of either timestep and particle)
                    #     write_scalers(self.writer, mean_noisy_probs.mean(0), self.global_step, f"validation/particle_mean")
                    #     # log the test results for each particle independently
                    #     if self.opt.log_particle:
                    #         for pi, probs_test in enumerate(all_noisy_probs):
                    #             for i, test_prob in enumerate(probs_test):
                    #                 write_scalers(self.writer_particle, test_prob, i, f"step_{self.global_step}/particle_{pi:02d}")
